...
export default function Settings() {
  ...
  useEffect(() => {
    ...
    setAutoSpeak(user.persona.auto_speak ?? false);
  ...
  const save = async () => {
    setSaving(true);
    try {
      const updated = await apiPatch<User>("/profile/persona", {
      ...
      <div className="mx-auto max-w-2xl">
        ...
        <section className={`mt-4 ${card}`} data-testid="models-section">
          ...
          <p className="mt-1 text-[12.5px] text-muted-foreground">
            VEX AI is the beta model. The backend remains the source of truth for its availability
            and credit usage.
          </p>
        ...
        <section className={`mt-4 ${card}`} data-testid="system-section">
          ...
          <dl className="mt-3 grid grid-cols-2 gap-y-2 text-[12.5px]">
            ...
            <dd data-testid="settings-email">Anonymous workspace</dd>
            ...
            <dd className={config?.provider_ready ? "text-[#3f6b45]" : "text-[#b8552f]"}>
              {config?.provider_ready ? "yes" : "no — configure the server provider"}
            </dd>
            {config &&
